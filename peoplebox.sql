...
select driver_name,round(avg(score),3) as avg_score from responses group by driver_name order by avg_score desc;

-- 2. Find all scores by department and driver combination.
select e.department,r.driver_name,round(avg(r.score),1) as avg_score from employees e join responses r on e.id=r.id group by e.department,r.driver_name;

-- 3. Find all scores by location and driver combination.
select e.location,r.driver_name,round(avg(r.score),1) as avg_score from employees e join responses r on e.id=r.id group by e.location,r.driver_name;

-- 4. Find all scores by gender and driver
select e.gender,r.driver_name,round(avg(r.score),1) as avg_score from employees e join responses r on e.id=r.id group by e.gender,r.driver_name;

-- 5. Find all scores by age group 20-30, 30-40,40-50.
select round(avg(case when e.age>=20 and e.age<30 then r.score end),2) as "20-30 age group",
round(avg(case when e.age>=30 and e.age<40 then r.score end),2) as "30-40 age group",
round(avg(case when e.age>=40 and e.age<=50 then r.score end),2) as "40-50 age group"
from employees e join responses r on e.id=r.id ;

-- 6. Find all scores by managers (ensure manager name is there)
select e.name,round(avg(r.score),2) as avg_score from employees e join employees m on e.id=m.manager_id join responses r on m.manager_id=r.id
group by e.name order by avg_score desc;
